#Cart算法与ID3，C4.5算法的区别：1.使用基尼系数而非熵来确定特征和取值，2.生成的树是一个二叉树(其中一个子节点必为叶节点)，其他两个不一定是二叉树
                            #3.CART可以分类也可以回归（包括了最小二乘回归树生成算法和CART生成算法）
                            #4.CART是选择基尼系数小的，其他两个是选信息增益(率)大的
#经验熵：-E(Pi*logPi)
#基尼系数:E(Pi*(1-Pi))=1-E(pow(Pi,2))
import cv2
import time
import logging
import numpy as np
...
if __name__ == '__main__':
    ...
    raw_data = pd.read_csv('G:/git_code/ML/LearningML/StatisticalLearningMethod/data/train_binary2.csv', header=0)
