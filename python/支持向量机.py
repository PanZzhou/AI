...
def kernelTrans(X, A, kTup): #核函数，输入参数,X:支持向量的特征树；A：某一行特征数据；kTup：('lin',k1)核函数的类型和参数
    m,n = shape(X)
    K = mat(zeros((m,1)))
    if kTup[0]=='lin': #线性函数,结果相当于感知机里的gram矩阵
        K = X * A.T
    elif kTup[0]=='rbf': # 径向基函数(radial bias function)
        for j in range(m):
            deltaRow = X[j,:] - A
            K[j] = deltaRow*deltaRow.T
        K = exp(K/(-1*kTup[1]**2)) #返回生成的结果,使用的是高斯核函数
    else:
...
def testRbf(data_train,data_test):
    ...
    for i in range(m):
        ...
        if sign(predict)==sign(labelArr[i]): #sign函数 -1 if x < 0, 0 if x==0, 1 if x > 0
            accuCount += 1
    print("the training accuracy rate is: %f" % (float(accuCount)/m)) #打印出错误率
    ...
    for i in range(m): #在测试数据上检验错误率
        ...
        if sign(predict)==sign(labelArr_test[i]):
            accuCount_test += 1
    print("the test accuracy rate is: %f" % (float(accuCount_test)/m))

#主程序
def main():
    filename_traindata='F:/code/sublimeCode/python/data/support_vector.csv'
    filename_testdata='F:/code/sublimeCode/python/data/support_vector1.csv'
    testRbf(filename_traindata,filename_testdata)

if __name__=='__main__':
    '''
	filename_traindata='F:\\code\\sublimeCode\\python\\data\\support_vector.csv'
	raw_data=pd.read_csv(filename_traindata,header=None,sep="\t")
	data=raw_data.values
	features=data[:,0:2]
	labels=data[:,2]
	red_labels=labels[labels==1]
	red_features=features[labels==1]
	plt.scatter(red_features[:,0],red_features[:,1],c='red')

	blue_lables=labels[labels==-1]
	blue_features=features[labels==-1]
	plt.scatter(blue_features[:,0],blue_features[:,1])
